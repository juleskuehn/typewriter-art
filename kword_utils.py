...
def chop_charset(fn='hermes.png', numX=79, numY=7, startX=0, startY=0, xPad=0, yPad=0, shrinkX=1, shrinkY=1, blankSpace=True, whiteThreshold=0.95,basePath='',excludeChars=[]):
    """
    ...
    for y in range(newStepY, im.shape[0], newStepY):
        for x in range(newStepX, im.shape[1], newStepX):
            if np.sum(im[y:y+newStepY, x:x+newStepX][:,:]) < (newStepX*newStepY*whiteThreshold*255):
                ...
                padded.append(im[y-yPad:y+newStepY+yPad, x-xPad:x+newStepX+xPad][:,:])
    # Append blank tile
    if blankSpace:
        padded.insert(0, np.full((newStepY+yPad*2, newStepX+xPad*2), 255.0, dtype='uint8'))
    xPad = 0
    yPad = 0
