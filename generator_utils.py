...
def putAnn(generator, row, col, mode='blend', put=True, whatsLeft=True, brighten=0):
    ...
    if brighten != 0:
        targetSlice = np.array(((targetSlice/255)*(1-brighten)+brighten)*255, dtype='uint8')
    if whatsLeft:
        # Remove existing placed ink from target
        targetSlice = compositeAdj(generator, row, col, targetSlice=targetSlice, subtractive=True)
    ...
    if mode == 'angular':
        ...
        for i, id in enumerate(angularScores[0]):
            ...
            d[id] += angularScores1[i] / boostEucl # Additive blend works better here.. normalization related?
...
def compositeAdj(generator, row, col, shrunken=False, targetSlice='', addFixed=True, subtractive=False):
